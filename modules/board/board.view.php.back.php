		...
		if ($result) {
			...
			if ($result) {
				...
				for ($i=0; $i<count($rows); $i++) {
					...
					$subject_obj['passover'] = $passover;
					$subject_obj['page'] = $page;
					...
					// 공지글 설정은 개발 예정 
					if (isset($is_notice) && $is_notice != '') {
						$subject_obj['space'] = '10px';
						$subject_obj['icon_box'] = '공지';
						$subject_obj['icon_box_color'] = 'icon-notice-color';
					}
					...
					$subject_obj['icon_opkey'] = $opkey;
					$subject_obj['icon_opkey_color'] = 'icon-opkey-color';
...
class DeletepassPanel extends BaseView {
	...
	function init() {
		...
		$skin_dir = "skin/$main_path";
		$skin_path = _SUX_PATH_ . "modules/board/$skin_dir/delpass.tpl";
		$default_header_path = _SUX_PATH_ . "modules/board/$skin_dir/_header.tpl";
		$default_footer_path = _SUX_PATH_ . "modules/board/$skin_dir/_footer.tpl";

		$document_data['skin_dir'] = $skin_dir;
		...
		if (is_readable($skin_path)) {
			...
			$smarty->assign('groupData', $group_data);
...
class DeletepassgrgPanel extends BaseView {
	...
	function init() {
		...
		$skin_dir = "skin/$main_path";
		$skin_path = _SUX_PATH_ . "modules/board/$skin_dir/delpass_grg.tpl";
		$default_header_path = _SUX_PATH_ . "modules/board/$skin_dir/_header.tpl";
		$default_footer_path = _SUX_PATH_ . "modules/board/$skin_dir/_footer.tpl";
...
class RecordDeletePanel extends BaseView {
	...
	function init() {
		...
		if ($pass == $row['pass'] || $pass == $admin_pwd) {
			...
			if(isset($del_filename) && $del_filename != '') {
