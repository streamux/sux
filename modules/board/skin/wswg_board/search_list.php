<?
$numresults=mysql_query("select * from $board where $find like '%$search%' order by id desc");
...
<table width="<? echo $width; ?>"  border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td valign="bottom"><table width="<? echo $width; ?>" height="27" border="0" cellpadding="0" cellspacing="0">
				<tr>
...
<?
$dbsearch="select * from $board where $find like '%$search%' order by id desc limit $passover,$limit";
$query=mysql_query($dbsearch);
...
$storytitle=StrCut($string, $num);
$storytitle=str_replace("$search","<font color=red>$search</font>",$storytitle);
...
?>
		...
		<td>
<?
	echo "<a href=board_read.php?board=$board&board_grg=$board_grg&id=$row[id]&igroup=$row[igroup]&passover=$passover&page=$page&sid=$sid&find=$find&search=$search&fmenu=$fmenu&fsubmenu=$fsubmenu&ljs_mod=r_mode&s_mod=s_mode>";
echo "$storytitle ";
echo "</a>";
...
?></td>
					...
					<td width="4"></td>
					<td width="90" align="center"><? echo $name; ?></td>
					<td width="4"></td>
...
<?
$underline=($numrows-$passover)-$list;
if($a > $underline ){
?>
				<tr>
...
echo "<font color=red>검색된 게시물이 없습니다.</font> ";
}
?>
			</table>
</td>
