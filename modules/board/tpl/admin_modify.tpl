    <div class="sx-content sx-admin-board">
      <section class="sx-add-panel">
        ...
        <div class="sx-box-content">                    
          <form action="{$rootPath}board-admin/modify" name="f_board_modify" enctype="multipart/form-data"  method="post" class="sx-form-horizontal">
            ...
            <div class="head_panel">
              <div class="sx-form-group">
                ...
                <select id="category" name="category" class="sx-form-control">
                  {foreach from=$documentData.board_list item=$item}
                    <option value="{$item.category}" {if $contentData.category == $item.category} selected="selected"{/if}>{$item.board_name}</option>
                  {/foreach}
                </select>
              </div>
              <div class="sx-form-inline">
                ...
                <div class="sx-input-group">
                  <input type="radio" id="isNoticeYes" name="is_notice" value="y" {if $contentData.is_notice == 'y'} checked="checked" {/if}>
                  <label for="isNoticeYes" class="sx-control-label sx-text-normal">출력</label>
                  <input type="radio" id="isNoticeNo" name="is_notice" value="n" {if $contentData.is_notice == 'n'} checked="checked" {/if}>
